    ...
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-primary mb-4">The Alexara Journal</h1>
          <p className="text-lg text-gray-600">Stories, guides, and inspiration for the modern traveler.</p>
        </div>
            ...
            <Link to={`/blog/${featuredPost.id}`} className="group block relative rounded-2xl overflow-hidden shadow-xl h-[500px]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-8 md:p-12">
                <div className="max-w-3xl">
                  ...
                  <span className="inline-flex items-center text-white font-bold border-b-2 border-secondary pb-1">Read Full Story <ArrowRight className="ml-2 w-5 h-5" /></span>
                </div>
              </div>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {remainingPosts.map((post) => (
            <article key={post.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow flex flex-col h-full">
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <Link to={`/blog/${post.id}`} className="text-primary font-bold text-sm hover:underline mt-auto">Read More</Link>
              </div>
            </article>
          ))}
        </div>
